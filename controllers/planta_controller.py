...
def index():
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("""SELECT p.idPlanta, p.nombreCientifico, p.linkImagen, fp.nomFamilia,
...
# Ruta principal para visualizar plantas
@planta_bp.route('/')
def index():
    try:
        conn = get_connection()
...
# Ruta para el registro de plantas
@planta_bp.route('/modoadmin')
def home():
...
# Detalles de una planta específica
