...
def crear_region():
    ...
    try:
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Región creada exitosamente"}
        ), 201
...
def actualizar_region(id_region):
    ...
    try:
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Región actualizada exitosamente"}
        )
...
def eliminar_region(id_region):
    ...
    try:
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Región eliminada exitosamente"}
        )
...
def crear_provincia():
    ...
    try:
        ...
        if not data or "nombreProvincia" not in data or "idRegion" not in data:
            return jsonify(
                {"error": 'Los campos "nombreProvincia" y "idRegion" son requeridos'}
            ), 400
        ...
        # Llamar al procedimiento almacenado para insertar
        cursor.callproc(
            "crud_provincias", [1, None, data["nombreProvincia"], data["idRegion"]]
        )
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Provincia creada exitosamente"}
        ), 201

    except mysql.connector.Error as e:
        error_msg = str(e)
        if "Esta provincia ya está registrada para esta misma región" in error_msg:
...
def actualizar_provincia(id_provincia):
    ...
    try:
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Provincia actualizada exitosamente"}
        )
...
def eliminar_provincia(id_provincia):
    ...
    try:
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Provincia eliminada exitosamente"}
        )
...
def asociar_provincia_ecoregion():
    ...
    try:
        ...
        if not data or "idProvincia" not in data or "idEcoregion" not in data:
            return jsonify(
                {"error": 'Los campos "idProvincia" y "idEcoregion" son requeridos'}
            ), 400
        ...
        cursor.callproc(
            "crud_provincia_ecoregion", [1, data["idProvincia"], data["idEcoregion"]]
        )
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Asociación creada exitosamente"}
        ), 201
...
def eliminar_provincia_ecoregion():
    ...
    try:
        ...
        # Llamar al procedimiento almacenado para eliminar
        cursor.callproc(
            "crud_provincia_ecoregion", [2, data["idProvincia"], data["idEcoregion"]]
        )
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Asociación eliminada exitosamente"}
        )
...
def asociar_ecoregion_planta():
    ...
    try:
        ...
        if not data or "idPlanta" not in data or "idEcoregion" not in data:
            return jsonify(
                {"error": 'Los campos "idPlanta" y "idEcoregion" son requeridos'}
            ), 400
        ...
        # Llamar al procedimiento almacenado para asociar
        cursor.callproc(
            "crud_ecoregion_planta", [1, None, data["idPlanta"], data["idEcoregion"]]
        )
...
@registro_zona.route(
    "/api/ecoregion-planta/actualizar/<int:id_ecoregion_planta>", methods=["PUT"]
)
def actualizar_ecoregion_planta(id_ecoregion_planta):
    ...
    try:
        ...
        # Llamar al procedimiento almacenado para actualizar
        cursor.callproc(
            "crud_ecoregion_planta", [2, id_ecoregion_planta, None, data["idEcoregion"]]
        )
        ...
        return jsonify(
            {
                "message": mensaje[0]
                if mensaje
                else "Asociación actualizada exitosamente"
            }
        )
...
@registro_zona.route(
    "/api/ecoregion-planta/eliminar/<int:id_ecoregion_planta>", methods=["DELETE"]
)
...
@registro_zona.route(
    "/api/ecoregion-planta/<int:id_ecoregion_planta>", methods=["DELETE"]
)
def archivar_ecoregion_planta(id_ecoregion_planta):
    ...
    try:
        ...
        return jsonify(
            {"message": mensaje[0] if mensaje else "Asociación archivada exitosamente"}
        )
