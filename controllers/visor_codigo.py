...
class FileSystemController:
    def __init__(self, project_root=None, allowed_folders=None):
        ...
        self.project_root = (
            os.path.abspath(project_root) if project_root else os.getcwd()
        )
    ...
    def is_path_allowed(self, path):
        ...
        for allowed_folder in self.allowed_folders:
            if path_parts[0] == allowed_folder or any(
                part == allowed_folder for part in path_parts
            ):
                return True
    ...
    def get_file_tree(self, root_path=None):
        ...
        if not self.is_path_allowed(abs_root):
            return {
                "error": "Acceso denegado: ruta fuera del proyecto o carpetas permitidas"
            }
                ...
                if isinstance(node, ast.Assign) and isinstance(
                    ...
                    if hasattr(node, "col_offset") and node.col_offset == 0:
                        ...
                        if not var_name.startswith(
                            "_"
                        ):  # Evitar variables privadas por defecto
                            elements.append(
                                {
                                    "name": var_name,
                                    "type": "variable",
    ...
    def get_file_content(self, file_path):
        ...
        if not self.is_path_allowed(file_path):
            return {
                "error": "Acceso denegado: archivo fuera del proyecto o carpetas permitidas"
            }
        ...
        try:
            ...
            if ext == ".md" or ext == ".markdown":
                ...
                html_content = markdown.markdown(
                    content, extensions=["codehilite", "fenced_code"]
                )
                return {
